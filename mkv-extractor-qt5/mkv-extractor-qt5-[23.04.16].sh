...
STR_LICENSE="MIT"
STR_MD5="8daaaf43c72963846482fa93b9be8038"
STR_FILENAME="v23.04.16.tar.gz"
STR_APPNAME="mkv-extractor-qt5"
STR_MAINTAINER="nsthy@hotmail.com"
STR_DESCRIPTION_SHORT="MKV Extracto Qt5 is written in python3 + Qt5 and works for Linux."
STR_DESCRIPTION="MKV Extracto Qt5 is written in python3 + Qt5 and works for Linux. It is to be used in addition to MKVToolNix."
#^^^^^^^^^^ EDIT ^^^^^^^^^^
STR_ARCH2=$(uname -m)
...
if [ "$(uname)" = "FreeBSD" ]; then
    	...
    	#vvvvvvvvvv EDIT vvvvvvvvvv
	STR_DEPEND_DEB="mkvtoolnix, qt5, python, python3, py39-qt5-pyqt, python39, ImageMagick7, git, ffmpeg, mkvalidator, mkclean"
	#^^^^^^^^^^ EDIT ^^^^^^^^^^
	...
	STR_PREFIX="usr"
	#vvvvvvvvvv EDIT vvvvvvvvvv
	STR_DEPEND_DEB="mkvtoolnix-gui, python3, libqt5core5a, libqt5gui5, libqt5widgets5, python3-pyqt5, imagemagick, git, ffmpeg"
	STR_DEPEND_RPM="mkvtoolnix qt5-qtbase python3 python3-qt5 ImageMagick git ffmpeg-free"
	STR_DEPEND_ARCHPKG="mkvtoolnix-gui qt5-base python python-pyqt5 imagemagick git ffmpeg mkvalidator mkclean"
	#^^^^^^^^^^ EDIT ^^^^^^^^^^
...
#vvvvvvvvvv EDIT vvvvvvvvvv
URL_HOMEPAGE="https://github.com/Hizoka76/MKV-Extractor-Qt5"
URL_DOWNLOAD="https://github.com/Hizoka76/MKV-Extractor-Qt5/archive/refs/tags/v${STR_VERSION}.tar.gz"

STR_ENV_DEPEND_FREEBSD="qt5 gcc py39-qt5-pyqt"
STR_ENV_DEPEND_DEBIAN="qtbase5-dev build-essential qttools5-dev-tools pyqt5-dev-tools"
STR_ENV_DEPEND_GENTOO="dev-qt/qtbase dev-qt/linguist-tools dev-qt/qttools"
STR_ENV_DEPEND_FEDORA="qt5-qtbase-devel python3-qt5 qt5-qttools qt5-linguist"
STR_ENV_DEPEND_ARCHLINUX="qt5-base base-devel qt5-tools python-pyqt5"
...
src_configure() {
	...
	# vvvvvvvvvv [DIY CODE] vvvvvvvvvv
	qt5_lrelease=$(which lrelease)
	if [ $? -eq 0 ]; then
		api_replace "/usr/lib/x86_64-linux-gnu/qt5/bin/lrelease" "$qt5_lrelease" "$DIR_HOME_WORKING/build.sh"
	else
		qt5_lrelease=$(which lrelease-qt5)
		if [ $? -eq 0 ]; then
			api_replace "/usr/lib/x86_64-linux-gnu/qt5/bin/lrelease" "$qt5_lrelease" "$DIR_HOME_WORKING/build.sh"
		else
			qt5_lrelease=$(find /usr -type f -name 'lrelease' | grep 'qt5')
			api_replace "/usr/lib/x86_64-linux-gnu/qt5/bin/lrelease" "$qt5_lrelease" "$DIR_HOME_WORKING/build.sh"
		fi
	fi
...
src_compile() {
	...
	if [ "$(uname)" = "FreeBSD" ]; then
		api_replace "sed -i " "sed -i '' " "build.sh"
	fi
	
	chmod +x build.sh
	api_verbose_run "bash build.sh"
	
	api_replace "Exec=mkv-extractor-qt5 %U" "Exec=python3 /opt/mkv-extractor-qt5/MKVExtractorQt5.py %U" "mkv-extractor-qt5.desktop"
	api_replace "TryExec=mkv-extractor-qt5" "" "mkv-extractor-qt5.desktop"
	api_replace "Icon=/usr/share/icons/hicolor/scalable/apps/mkv-extractor-qt5.svg" "Icon=mkv-extractor-qt5" "mkv-extractor-qt5.desktop"
...
src_pack() {
	...
	cd $DIR_HOME_WORKING
	mkdir tmpfolder
	mv ui_MKVExtractorQt5.ui tmpfolder
	mv ui_MKVExtractorQt5.py tmpfolder
	mv MKVRessources_rc.py tmpfolder
	mv MKVRessources.qrc tmpfolder
	mv CodecListFile.py tmpfolder
	mv Languages tmpfolder
	mv QFileDialogCustom tmpfolder
	mv WhatsUp tmpfolder
	mv man tmpfolder
	mv img tmpfolder
	mv MKVExtractorQt5.py tmpfolder
	cd $DIR_HOME
	
	api_deb_install_opt_sub "$DIR_HOME_WORKING/tmpfolder" "mkv-extractor-qt5"
	api_deb_install_icon $DIR_HOME_WORKING/icons/mkv-extractor-qt5.svg
	api_deb_install_desktop $DIR_HOME_WORKING/mkv-extractor-qt5.desktop
