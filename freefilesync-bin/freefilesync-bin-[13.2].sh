...
STR_LICENSE="MIT"
STR_MD5="9eb9ae21cf02fca677a429714a8cecd7"
STR_FILENAME="FreeFileSync_13.2_Linux.tar.gz"
STR_APPNAME="freefilesync-bin"
STR_MAINTAINER="nsthy@hotmail.com"
STR_DESCRIPTION_SHORT="Cross-platform file sync utility (GPL release)"
STR_DESCRIPTION="Cross-platform file sync utility (GPL release)
...
if [ "$(uname)" = "FreeBSD" ]; then
    	...
    	#vvvvvvvvvv EDIT vvvvvvvvvv
	STR_DEPEND_DEB="linux-c7, linux-c7-gtk3"
	#^^^^^^^^^^ EDIT ^^^^^^^^^^
	...
	STR_PREFIX="usr"
	#vvvvvvvvvv EDIT vvvvvvvvvv
	STR_DEPEND_DEB="xdg-utils, libc6, libcurl4, libgcc-s1, libgdk-pixbuf-2.0-0, libglib2.0-0, libgtk-3-0, libselinux1, libssh2-1, libssl3, libstdc++6, libwxbase3.2-1, libwxgtk3.2-1, zlib1g, libgtk2.0-0"
	STR_DEPEND_RPM="wxGTK libstdc++ gtk3 gdk-pixbuf2 glib2 libselinux zlib gtk2"
	STR_DEPEND_ARCHPKG="wxwidgets-gtk3 gcc-libs gtk3 gdk-pixbuf2 glib2 zlib gtk2"
	#^^^^^^^^^^ EDIT ^^^^^^^^^^
...
#vvvvvvvvvv EDIT vvvvvvvvvv
URL_HOMEPAGE="https://freefilesync.org/"
URL_DOWNLOAD="https://freefilesync.org/download/${STR_FILENAME}"
...
src_extract() {
	 ...
	 api_file_extract
	# vvvvvvvvvv [DIY CODE] vvvvvvvvvv
	local tmp_offset=$(grep -abo -m 1 -F "<FFS_TAR_START>" "${DIR_WORKING}/FreeFileSync_${STR_VERSION}_Install.run" | cut -d : -f 1)
	 tmp_offset=$((tmp_offset + 16))
	 tail -c +$tmp_offset "${DIR_WORKING}/FreeFileSync_${STR_VERSION}_Install.run" > "${DIR_WORKING}/FreeFileSync_run_extract.tar"
	 api_mkdir ${DIR_WORKING}/extract_phase1
	 tar -xf $DIR_WORKING/FreeFileSync_run_extract.tar -C $DIR_WORKING/extract_phase1 > /dev/null 2>&1
	 api_file_extract_status_check "$DIR_WORKING/FreeFileSync_run_extract.tar"
	 api_mkdir $DIR_WORKING/extract_phase1/extract_phase2
	 tar -xvf $DIR_WORKING/extract_phase1/FreeFileSync.tar.gz -C $DIR_WORKING/extract_phase1/extract_phase2 > /dev/null 2>&1
	 api_file_extract_status_check "$DIR_WORKING/extract_phase1/FreeFileSync.tar.gz"
...
src_configure() {
	...
	# vvvvvvvvvv [DIY CODE] vvvvvvvvvv
	api_replace "FFS_INSTALL_PATH" "/opt/FreeFileSync" "${DIR_WORKING}/extract_phase1/FreeFileSync.template.desktop"
	api_replace "FFS_INSTALL_PATH" "/opt/FreeFileSync" "${DIR_WORKING}/extract_phase1/RealTimeSync.template.desktop"
	
	mv ${DIR_WORKING}/extract_phase1/FreeFileSync.template.desktop ${DIR_WORKING}/extract_phase1/FreeFileSync.desktop
	mv ${DIR_WORKING}/extract_phase1/RealTimeSync.template.desktop ${DIR_WORKING}/extract_phase1/RealTimeSync.desktop
...
src_pack() {
	...
	# vvvvvvvvvv [DIY CODE] vvvvvvvvvv
	api_deb_install_opt_sub "$DIR_HOME/$DIR_WORKING/extract_phase1/extract_phase2" "FreeFileSync"
	api_deb_install_icon $DIR_WORKING/extract_phase1/extract_phase2/Resources/FreeFileSync.png
	api_deb_install_icon $DIR_WORKING/extract_phase1/extract_phase2/Resources/RealTimeSync.png
	api_deb_install_desktop $DIR_WORKING/extract_phase1/FreeFileSync.desktop
	api_deb_install_desktop $DIR_WORKING/extract_phase1/RealTimeSync.desktop
